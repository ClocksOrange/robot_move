...
void init_markers(visualization_msgs::Marker *marker)
{
  ...
  marker->lifetime = ros::Duration(240);//0 is forever
  marker->scale.x  = 0.2;
  marker->scale.y  = 0.2;
  marker->color.r  = 1.0;
  marker->color.g  = 0.7;
...
{
  ros::init(argc, argv, "move_base_square");
 // std::string topic = "/move_base_simple/goal";
  ros::NodeHandle node;
  ...
  point.x = -2.04;//square_size
  point.y = -2.01;
  point.z = 0.0;
  pose_list[0].position = point;
  pose_list[0].orientation = quaternions[0];

  point.x = 1.92;//square_size
  point.y = -1.92;//square_size
  point.z = 0.0;
 ...
 // cmdVelPub = node.advertise<geometry_msgs::Twist>(topic, 5);//????
  ...
  while( (count < 4) && (ros::ok()) )
  {
    ...
    else
    {
	 ...
	 ROS_INFO("this is %d point",count+1);
     count += 1;
  }
